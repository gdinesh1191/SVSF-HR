...
class MeekroDB {
  ...
  public function insertOrReplace($which, $table, $datas, $options=array()) {
   	 global $user_info;   global $main_db;   
	 $user_info['id'] = isset($user_info['id']) ? $user_info['id'] : 0; 
	
	 $db =($this->dbName==NULL)?  $main_db:$this->dbName;	 
	  $details = DB::query("SELECT COLUMN_NAME,DATA_TYPE,COLUMN_KEY 	FROM INFORMATION_SCHEMA.COLUMNS 
							WHERE  TABLE_SCHEMA = '".$db."' AND TABLE_NAME = '".$table."'  ");
	foreach($details as  $list)	 
	{ 
		  $data_type[$list['COLUMN_NAME']]["key"] = $list['COLUMN_KEY']; 
		  $data_type[$list['COLUMN_NAME']]["type"] = $list['DATA_TYPE'];
 	 } 		
 	 
	$column_list = DB::columnList($table); 	
	foreach($column_list as $list) 
	{ 
		if($data_type[$list]["key"]=="PRI")  { $primary_key = $list;  } 
		
		if($data_type[$list]["key"]=="PRI" || $data_type[$list]["type"]=="int" || $data_type[$list]["type"]=="float" )
		{ 
			 $datas[$list] = trim($datas[$list]);
			 $datas[$list] = ($datas[$list]==NULL)? 0:$datas[$list]; 			 
		}
		else $datas[$list] = ($datas[$list]==NULL)? "":$datas[$list]; 		
	}   
 
	$datas['user'] = $user_info['id']; $datas['update_user'] = $user_info['id'];  
	$datas['creation_date'] =date("d/m/Y h:i:s A");     $datas['update_date'] =date("d/m/Y h:i:s A"); 
  ...
  public function insertUpdate() {
  	global $user_info;   $user_info['id'] = isset($user_info['id']) ? $user_info['id'] : 0; 
    $args = func_get_args();
    $table = array_shift($args);
    ...
    if (is_array($args[0])) $update = $args[0];
   	...
   	$update['update_date'] = date("d/m/Y h:i:s A");
	$update['update_user'] = $user_info['id'];
  ...
  public function delete() {    
	...
	$data = DB::query("select * from  $table  WHERE    ".str_replace("%i","",$where)." ".(isset($args[0]) ? $args[0] : "")."  ");
 	foreach($data as $list)
	{
		 DB::insertUpdate("zd_".str_replace('`','',$table), $list);
	}
  ...
  public function query() { 
    $args = func_get_args();    
    $_data = $this->prependCall(array($this, 'queryHelper'), $args, 'assoc');  
    
    // Handle missing getArrayRowsColumns function
    $dimensions = array("rows" => 0, "columns" => 0);
    if (is_array($_data)) {
      $dimensions["rows"] = count($_data);
      if ($dimensions["rows"] > 0 && is_array($_data[0])) {
        $dimensions["columns"] = count($_data[0]);
      }
    }
    
    $size = ($dimensions["rows"] * $dimensions["columns"]);
	
	$data_string = array("size" => $size, "url" => isset($_SERVER['HTTP_HOST']) ? "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'] : '', "query" => $args[0], "date" => date("d/m/y H:i:s A"));
   
   //if($size>5000) { file_put_contents('logs.txt', ','.json_encode($data_string) , FILE_APPEND | LOCK_EX);  }
	if($size>5000) 
	{
		$ch = curl_init('https://infogreen.in/api/mysql.php');
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "POST");
		curl_setopt($ch, CURLOPT_POSTFIELDS,json_encode($data_string)); // Use $data_string directly
...
function meekrodb_error_handler($params) {
  ...
  $out[] = "";
	...
	if(isset($params['all']['args'][1]) && $params['all']['args'][1]=="INSERT") { 
        $columns = $params['all']['args'][3]; 
        $table = $params['all']['args'][2];
    } else { 
        $columns = isset($params['all']['args'][2]) ? $params['all']['args'][2] : null; 
        $table = isset($params['all']['args'][1]) ? $params['all']['args'][1] : null;
    }
	$code = isset($params['code']) ? $params['code'] : null;
	
	if($table != NULL) {
 		($code==1146)? create_table($table,$columns):"";
		($code==1054)? alter_table($table,$columns):"";		
		 DB::query(isset($params['query']) ? $params['query'] : "");
	}   
	$txt = date("d/m/Y h:i:s ").PHP_EOL;
   if (php_sapi_name() == 'cli' && empty($_SERVER['REMOTE_ADDR'])) 
   {  
   	//echo  '<br><br><br>'.implode("\n", $out);  
	} 
	else 
	{  
...
{   
    $query = 'CREATE TABLE `'.$table.'` ( `id` int(11) NOT NULL AUTO_INCREMENT,   ';  $count =1;
    foreach($columns as $list) {  if(trim($list)!="id") {  $query .=  '`'. $list.'` text ,  '; $count++; }    }
    $query .= '  PRIMARY KEY (`id`) )    ENGINE=MyISAM AUTO_INCREMENT=3 DEFAULT CHARSET=latin1;' ;
    if($count>=2) { DB::query($query); }
} 

function alter_table($table,$columns_fixed)  
{   
$columns = DB::columnList($table); 
$result = array_diff($columns_fixed, $columns);
foreach($result as $list) {   
    DB::query("ALTER TABLE `".$table."` ADD `".$list."` text ;");
}
} 
